...
function parsePositiveIntOrDefault(raw, def) {
...
function runDemo() {
    ...
    const angleThreshold = Number.isFinite(Number(process.argv[5])) ? Number(process.argv[5]) : 90;
    // --- РІВЕНЬ 1 ---
    console.log("=== ВИКОНАННЯ РІВНЯ 1 (Без колізій) ===");
    console.log(`Розмір таблиці: ${size}`);
    const ht1 = new HashTableL1(size);
    // Вставляємо так, щоб НЕ було колізій: генеруємо, доки insert() не поверне true
    let inserted1 = 0;
    let tries1 = 0;
    while (inserted1 < Math.min(nLevel1, size) && tries1 < 10000) {
        tries1++;
        if (ht1.insert(Triangle.generateRandom()))
            inserted1++;
    }
    ht1.print("Таблиця 1-го рівня");
    // --- РІВЕНЬ 2 ---
    console.log("\n=== ВИКОНАННЯ РІВНЯ 2 (Квадратичне зондування) ===");
    const ht2 = new HashTableL2(size);
    // Вставляємо 8 елементів, щоб спровокувати колізії
    for (let i = 0; i < Math.min(nLevel23, size); i++) {
        ht2.insert(Triangle.generateRandom());
    }
    ht2.print("Таблиця 2-го рівня");
    // --- РІВЕНЬ 3 ---
    console.log("\n=== ВИКОНАННЯ РІВНЯ 3 (Видалення за критерієм) ===");
