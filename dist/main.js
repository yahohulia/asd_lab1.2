...
async function main() {
    ...
    console.log("=== Хеш-таблиця: Відрізок / Метод множення / Квадратичне зондування ===\n");
    const args = process.argv.slice(2);
    let size, nL1, nL23, angleThreshold;
    if (args.length >= 4) {
        const a0 = args[0];
        const a1 = args[1];
        const a2 = args[2];
        ...
        rl.close();
        console.log(`Параметри (з аргументів): size=${size}, nL1=${nL1}, nL23=${nL23}, angle>${angleThreshold}°\n`);
    }
    else {
        const rawSize = await askQuestion(rl, "Введіть розмір хеш-таблиці: ");
        size = Math.max(1, parseInt(rawSize, 10) || 10);
        const rawN1 = await askQuestion(rl, "Кількість елементів для рівня 1 (без колізій): ");
        nL1 = Math.min(Math.max(1, parseInt(rawN1, 10) || 5), size);
        const rawN23 = await askQuestion(rl, "Кількість елементів для рівнів 2 і 3: ");
        nL23 = Math.min(Math.max(1, parseInt(rawN23, 10) || 8), size);
        const rawAngle = await askQuestion(rl, "Поріг кута (°) для видалення (рівень 3): ");
    ...
    while (inserted1 < nL1 && tries1 < 100_000) {
        ...
        if (ok) {
            ...
            console.log(`  [-] Колізія — позиція зайнята, елемент відхилено: ${seg.toString()}`);
        }
    }
    ...
    for (let i = 0; i < nL23; i++) {
        ...
        console.log(ok ? `  [+] Вставлено: ${seg.toString()}` : `  [!] Таблиця переповнена.`);
    }
    ht2.print("Таблиця рівня 2");
    console.log("\n═══════════════════════════════════════════════════════════");
    console.log("  РІВЕНЬ 3 — Видалення елементів з |кут > OX| > порогу");
    console.log("═══════════════════════════════════════════════════════════");
    console.log(`Розмір таблиці: ${size}  |  Елементів: ${nL23}  |  Поріг: ${angleThreshold}°`);
